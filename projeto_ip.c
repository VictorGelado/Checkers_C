...
int verifyValidMove(int cp, int jp, int cp2, int jp2, int ch[CHECKERSLEN][CHECKERSLEN]) {
    ...
    int previousSquareC = cp-1;
    int previousSquareJ = jp-1;
...
void makeMove(int ch[CHECKERSLEN][CHECKERSLEN], int *whoseTurn) {
    ...
    do {
        do {
            ...
            displayCheckers(ch);
            printf("Digite a linha e coluna para onde você quer mover a peça (separado por espaço): ");
            scanf("%d %d", &cp2, &jp2);
